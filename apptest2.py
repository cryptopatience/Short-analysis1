...
# ✅ 종목 병합 함수 추가
def merge_custom_stocks(base_stocks, custom_input):
    """기본 종목 + 사용자 입력 종목 병합"""
    merged = base_stocks.copy()
    ...
    if custom_input and custom_input.strip():
        ...
        for ticker in tickers:
            if ticker and ticker not in merged:
                # 자동으로 기본 정보 생성
                merged[ticker] = {
                    'name': ticker,  # 티커명을 회사명으로 사용
                    'description': f'{ticker} 종목 (사용자 추가)',
                    'sector': 'Unknown',
                    'industry': 'Unknown'
                }
...
# ✅ 전역 변수로 현재 종목 리스트 관리
MAG7_STOCKS = DEFAULT_MAG7_STOCKS.copy()
...
def get_quarterly_vwap_analysis(ticker):
    ...
    try:
        ...
        # ✅ 종목 정보 안전하게 가져오기
        stock_info = MAG7_STOCKS.get(ticker, {
            'name': ticker,
            'description': 'Unknown',
            'sector': 'Unknown',
            'industry': 'Unknown'
...
def collect_all_data():
    ...
    mag7_tickers = list(MAG7_STOCKS.keys())  # ✅ 동적으로 변경됨
    ...
    for idx, ticker in enumerate(mag7_tickers):
        ...
        status_text.text(f"분석 중: {ticker} ({stock_info['name']})...")
...
def main():
    ...
    with st.sidebar:
        ...
        custom_tickers = st.text_input(
            "추가 분석 종목 (쉼표로 구분)",
            placeholder="예: AMD, PLTR, ETH-USD",
            help="야후 파이낸스 티커 형식으로 입력하세요"
        )
    
        # ✅ 최종 종목 리스트 병합
        global MAG7_STOCKS  # 전역 변수 사용
        MAG7_STOCKS = merge_custom_stocks(DEFAULT_MAG7_STOCKS, custom_tickers)
    
        # ✅ 현재 분석 대상 종목 표시
        with st.expander(f"📋 분석 대상 ({len(MAG7_STOCKS)}개 종목)"):
            base_tickers = list(DEFAULT_MAG7_STOCKS.keys())
            for ticker, info in MAG7_STOCKS.items():
                if ticker in base_tickers:
                    st.text(f"• {info['name']} ({ticker})")
                else:
                    st.text(f"🆕 {info['name']} ({ticker})")
        ...
        if st.button("🔄 데이터 새로고침", use_container_width=True):
            st.cache_data.clear()
            st.session_state['analysis_data'] = None  # ✅ 캐시 초기화
            st.rerun()
    ...
    # 데이터 로드
    # ✅ 데이터 로드 - 커스텀 종목 반영
    # 종목 리스트가 변경되었거나, 캐시가 없으면 새로 수집
    current_tickers = set(MAG7_STOCKS.keys())
    cached_tickers = set(st.session_state.get('cached_tickers', []))

    if (st.session_state.get('analysis_data') is None or 
        current_tickers != cached_tickers):
    
        with st.spinner(f"데이터 수집 중... ({len(MAG7_STOCKS)}개 종목)"):
            st.session_state['analysis_data'] = collect_all_data()
            st.session_state['cached_tickers'] = list(current_tickers)
