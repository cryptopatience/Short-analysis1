...
def create_gemini_prompt(df_results, analysis_type):
    ...
    # 데이터 요약
    data_summary = f"""
현재 MAG 7+2 종목 분석 데이터:

전체 평균 투자 점수: {df_results['Total_Investment_Score'].mean():.1f}/120
VWAP 위 거래 종목: {df_results['Is_Above_VWAP'].sum()}개 / {len(df_results)}개
평균 공매도 비율: {df_results['short_percent_float'].mean():.2f}%

상위 5개 종목:
{df_results.head(5)[['Ticker', 'Company', 'Current_Price', 'Price_vs_VWAP_%', 'Quarter_Return_%', 'short_percent_float', 'Total_Investment_Score']].to_string()}
"""
    
    if analysis_type == "basic":
        prompt = f"""
당신은 전문 퀀트 애널리스트입니다. 다음 MAG 7+2 종목 데이터를 분석하고 투자 인사이트를 제공하세요.
...
한국어로 답변하고, 전문적이면서도 이해하기 쉽게 작성하세요.
"""
    
    else:  # deep dive
        prompt = f"""
당신은 전문 퀀트 애널리스트입니다. 다음 MAG 7+2 종목 데이터를 심층 분석하세요.

{data_summary}

다음 형식으로 상세 분석을 제공하세요:
...
한국어로 전문적이고 상세하게 작성하세요.
"""
    
    return prompt


def analyze_with_gemini_demo(df_results, analysis_type="basic"):
    """Gemini 데모 모드 (API 실패 시 폴백)"""
    time.sleep(1)  # 시뮬레이션
...
**종합 시장 전망:**
현재 MAG 7+2 포트폴리오는 혼조세를 보이고 있습니다. 
상위 3개 종목 ({', '.join(df_results.head(3)['Ticker'].tolist())})이 
강한 모멘텀을 보이며 총점 {df_results.head(3)['Total_Investment_Score'].mean():.1f}/120을 기록했습니다.

**Top Pick:** {df_results.iloc[0]['Ticker']} - {df_results.iloc[0]['Company']}
- 현재가: ${df_results.iloc[0]['Current_Price']}
- VWAP 대비: {df_results.iloc[0]['Price_vs_VWAP_%']:+.2f}%
- 공매도 비율: {df_results.iloc[0]['short_percent_float']:.2f}%
- 종합 점수: {df_results.iloc[0]['Total_Investment_Score']}/120

**투자 전략:**
단기적으로는 VWAP 위에서 거래되는 종목들의 모멘텀 전략을 추천하며,
공매도 비율이 5% 미만인 종목들을 중심으로 포트폴리오를 구성하시기 바랍니다.
"""
    else:  # deep dive
        analysis = f"""
### 🔬 Gemini AI Deep Dive 분석
...
**VWAP 위 거래**: {df_results['Is_Above_VWAP'].sum()}개

---

## 1️⃣ 거시경제 관점
현재 빅테크 기업들은 AI 투자 확대로 장기 성장 모멘텀 유지.  
연준 금리 정책 불확실성에도 펀더멘털 견고.
...
📊 **기술적**: 현재가 ${row['Current_Price']} (VWAP {row['Price_vs_VWAP_%']:+.2f}%), 분기수익 {row['Quarter_Return_%']:+.2f}%  
🔴 **공매도**: {row['short_percent_float']:.2f}% ({"안전" if row['short_percent_float'] < 5 else "주의"})  
💡 **추천**: {"🟢 적극 매수" if row['Total_Investment_Score'] >= 90 else "🟡 매수" if row['Total_Investment_Score'] >= 75 else "⚪ 관찰"}  
📈 **Entry**: ${row['Current_Price']:.2f} | **Target**: ${row['Current_Price'] * 1.10:.2f} | **Stop**: ${row['Anchored_VWAP']:.2f}
...
### 공격적 (30%+ 수익 목표)
{chr(10).join([f"- {row['Ticker']}: 30%" for idx, row in df_results.head(2).iterrows()])}
- 현금: 40%

### 균형 (15%+ 수익 목표)
{chr(10).join([f"- {row['Ticker']}: 20%" for idx, row in df_results.head(5).iterrows()])}

### 보수적 (8%+ 수익 목표)
{chr(10).join([f"- {row['Ticker']}: 20%" for idx, row in df_results[df_results['short_percent_float'] < 3].head(3).iterrows()])}
- 현금: 40%
...
### 즉시 매수 (Strong Buy)
{chr(10).join([f"**{row['Ticker']}**: Entry ${row['Current_Price']:.2f}, Target ${row['Current_Price'] * 1.05:.2f}, Stop ${row['Anchored_VWAP']:.2f}" for idx, row in df_results[df_results['Total_Investment_Score'] >= 90].head(3).iterrows()])}

### 조정 시 매수 (Buy on Dip)
{chr(10).join([f"**{row['Ticker']}**: 대기가 ${row['Anchored_VWAP']:.2f} (VWAP)" for idx, row in df_results[(df_results['Total_Investment_Score'] >= 75) & (df_results['Total_Investment_Score'] < 90)].head(2).iterrows()])}

---

## 5️⃣ 리스크 관리
- **손절**: VWAP 이탈 시 즉시 / -5% 도달 시 검토
- **비중**: 단일 종목 최대 30%
- **리밸런싱**: 월 1회
...
## 6️⃣ 최종 권장사항
**즉시**: Top 3 ({', '.join(df_results.head(3)['Ticker'].tolist())}) 비중 확대  
**이번 주**: 포트폴리오 리밸런싱  
**이번 달**: 실적 발표 체크

**다음 업데이트**: {(datetime.now() + timedelta(days=7)).strftime('%Y-%m-%d')}

_투자는 본인 판단과 책임 하에 이루어져야 합니다._
"""
...
def create_openai_prompt(df_results, analysis_type):
    ...
    # 데이터 JSON 형식
    data_json = df_results.head(9)[['Ticker', 'Company', 'Current_Price', 'Anchored_VWAP', 
                                     'Price_vs_VWAP_%', 'Quarter_Return_%', 
                                     'short_percent_float', 'Buy_Signal_Score', 
                                     'Total_Investment_Score']].to_dict('records')
    
    if analysis_type == "basic":
        prompt = f"""
다음은 MAG 7+2 종목의 현재 분석 데이터입니다:

{data_json}

다음 형식으로 기본 분석을 제공하세요:
...
2. 공매도 심층 분석 (분포 통계)
3. 상관관계 분석
4. 포트폴리오 최적화 제안 (공격적/균형/보수적)
5. 매매 시그널 (Entry/Target/Stop Loss)
6. 리스크 관리 전략

구체적인 수치와 코드 블록을 포함하여 작성하세요.
한국어로 상세하게 작성하세요.
"""
...
def analyze_with_openai_demo(df_results, analysis_type="basic"):
    """OpenAI 데모 모드 (API 실패 시 폴백)"""
    time.sleep(1)  # 시뮬레이션
...
**시장 개요:**
MAG 7+2 종목들의 현재 테크니컬 분석 결과, 
평균 투자 점수는 {df_results['Total_Investment_Score'].mean():.1f}/120입니다.

**최고 추천 종목:**
1위: {df_results.iloc[0]['Ticker']} (점수: {df_results.iloc[0]['Total_Investment_Score']})
2위: {df_results.iloc[1]['Ticker']} (점수: {df_results.iloc[1]['Total_Investment_Score']})
3위: {df_results.iloc[2]['Ticker']} (점수: {df_results.iloc[2]['Total_Investment_Score']})

**공매도 리스크 평가:**
- 안전 종목 (<5% 공매도): {len(df_results[df_results['short_percent_float'] < 5])}개
- 주의 종목 (≥10% 공매도): {len(df_results[df_results['short_percent_float'] >= 10])}개

**행동 가이드:**
90점 이상 종목은 즉시 매수, 75-90점은 조정 시 매수를 권장합니다.
"""
    else:  # deep dive
        analysis = f"""
### 🔬 GPT-4 Deep Dive 분석
...
**1. VWAP 모멘텀 분석**
```
VWAP 위 거래 종목: {len(df_results[df_results['Is_Above_VWAP']])}개
평균 VWAP 대비 거리: {df_results['Price_vs_VWAP_%'].mean():+.2f}%
최고 모멘텀: {df_results.iloc[0]['Ticker']} ({df_results.iloc[0]['Price_vs_VWAP_%']:+.2f}%)
```

**2. 공매도 심층 분석**
```python
# 공매도 비율 분포
매우 낮음 (<2%): {len(df_results[df_results['short_percent_float'] < 2])}개
낮음 (2-5%): {len(df_results[(df_results['short_percent_float'] >= 2) & (df_results['short_percent_float'] < 5)])}개
보통 (5-10%): {len(df_results[(df_results['short_percent_float'] >= 5) & (df_results['short_percent_float'] < 10)])}개
높음 (≥10%): {len(df_results[df_results['short_percent_float'] >= 10])}개
```

**3. 상관관계 분석**
VWAP 강도와 공매도 비율 간 음의 상관관계 확인됨.
강한 기술적 모멘텀을 가진 종목일수록 공매도 비율이 낮은 경향.

**4. 포트폴리오 최적화 제안**

**공격적 포트폴리오 (High Risk/High Return)**
{chr(10).join([f"- {row['Ticker']}: 30%" for idx, row in df_results.head(2).iterrows()])}
- 현금: 40%

**균형 포트폴리오 (Balanced)**
{chr(10).join([f"- {row['Ticker']}: {100//5}%" for idx, row in df_results.head(5).iterrows()])}

**보수적 포트폴리오 (Low Risk)**
{chr(10).join([f"- {row['Ticker']}: {60//3}%" for idx, row in df_results[df_results['short_percent_float'] < 3].head(3).iterrows()])}
- 현금: 40%

**5. 매매 시그널**
{chr(10).join([f'''
{row['Ticker']}: {"🟢 BUY" if row['Total_Investment_Score'] >= 90 else "🟡 HOLD" if row['Total_Investment_Score'] >= 75 else "⚪ WATCH"}
  - Entry: ${row['Current_Price']:.2f}
  - Target: ${row['Current_Price'] * 1.05:.2f} (+5%)
  - Stop Loss: ${row['Anchored_VWAP']:.2f} (VWAP)
''' for idx, row in df_results.head(5).iterrows()])}

**6. 리스크 관리**
- 개별 종목 비중 30% 이하 유지
- VWAP 아래로 이탈 시 즉시 손절
- 공매도 비율 급증 종목 모니터링 필수
"""
